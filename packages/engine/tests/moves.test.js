...
describe('legalMoves — direction restrictions', () => {
    ...
    it('kings can move both directions by default', () => {
        const state = makeState([{ row: 4, col: 3, side: 'player', king: true }], 'player');
        const moves = legalMoves(state);
...
describe('legalMoves — blocked squares', () => {
    ...
    it('returns empty list when game is not active', () => {
        const state = makeState([{ row: 5, col: 2, side: 'player' }], 'player', { status: 'won' });
...
describe('applyMove', () => {
    ...
    it('throws when game is not active', () => {
        const state = makeState([{ row: 5, col: 2, side: 'player' }], 'player', { status: 'draw' });
        const move = {
            from: [5, 2],
            to: [4, 3],
            steps: [[4, 3]],
...
//# sourceMappingURL=moves.test.js.map