...
describe('marks accumulation', () => {
  it.skipIf(!dbAvailable)('awards 1 mark per Unbaked win, passes level on 2nd', async () => {
    const r1 = await playWinningMove('marks-1', 'unbaked');
    expect(r1.markAwarded).toBe(true);
    expect(r1.marksTotal).toBe(1);
    expect(r1.marksRequired).toBe(2); // Unbaked threshold (Phase 5.0.4)
    ...
    expect(r2.levelPassed).toBe(true);
  });

  it.skipIf(!dbAvailable)('REGRESSION: 3 Sheriff + 1 Unbaked does NOT pass', async () => {
    // Three Sheriff wins (path threshold is 4, not yet reached).
    const s1 = await playWinningMove('marks-mixed', 'sheriff');
    expect(s1.marksTotal).toBe(1);
    expect(s1.marksRequired).toBe(4);
    ...
    const s3 = await playWinningMove('marks-mixed', 'sheriff');
    expect(s3.marksTotal).toBe(3);
    expect(s3.levelPassed).toBe(false);

    // One Unbaked win (path threshold is 2, not yet reached at 1).
    const u1 = await playWinningMove('marks-mixed', 'unbaked');
    expect(u1.marksTotal).toBe(1); // Per-opponent count, NOT cross-path sum
    expect(u1.marksRequired).toBe(2);
    expect(u1.levelPassed).toBe(false);

    // Verify via /marks endpoint that paths are tracked separately.
    // Total wins: 3+1=4 — same as Sheriff threshold — but MUST NOT pass.
    const marks = await getUserMarks('marks-mixed');
    expect(marks.paths.sheriff.marks).toBe(3);
    expect(marks.paths.sheriff.required).toBe(4);
    expect(marks.paths.sheriff.passed).toBe(false);
    expect(marks.paths.unbaked.marks).toBe(1);
    expect(marks.paths.unbaked.required).toBe(2);
  ...
  it.skipIf(!dbAvailable)('Sheriff and Unbaked progress remain separate', async () => {
    // 3 sheriff wins (still below 4-win threshold)
    for (let i = 0; i < 3; i++) {
      const r = await playWinningMove('marks-sep', 'sheriff');
      expect(r.levelPassed).toBe(false);
    }
    // 1 unbaked win (still below 2-win threshold)
    const r4 = await playWinningMove('marks-sep', 'unbaked');
    expect(r4.levelPassed).toBe(false);

    // Total wins: 4, but no path passed yet (combined sum doesn't count).
    const marks = await getUserMarks('marks-sep');
    expect(marks.paths.sheriff.marks).toBe(3);
    expect(marks.paths.unbaked.marks).toBe(1);
    expect(marks.levelPassed).toBe(false);

    // One more unbaked win clears the Unbaked path (2/2) but not Sheriff (3/4).
    const last = await playWinningMove('marks-sep', 'unbaked');
    expect(last.levelPassed).toBe(true);
    expect(last.marksTotal).toBe(2);
    expect(last.marksRequired).toBe(2);
  ...
  it.skipIf(!dbAvailable)('passing Sheriff requires 4 Sheriff wins (not 4 total)', async () => {
    // Mix path wins: 2 Unbaked + 2 Sheriff = 4 total wins. Unbaked passes
    // at 2/2; Sheriff still 2/4. The sum equaling 4 is irrelevant.
    for (let i = 0; i < 2; i++) await playWinningMove('marks-sheriff', 'unbaked');
    for (let i = 0; i < 2; i++) await playWinningMove('marks-sheriff', 'sheriff');

    const after = await getUserMarks('marks-sheriff');
    expect(after.paths.unbaked.passed).toBe(true); // unbaked done at 2/2
    expect(after.paths.sheriff.passed).toBe(false); // sheriff: 2/4
    // levelPassed is true because at least one path passed (Unbaked) —
    // it's NOT a 4-total-wins gate.
    expect(after.levelPassed).toBe(true);

    // Adding 2 more Sheriff wins (total 4 Sheriff) finally passes Sheriff.
    for (let i = 0; i < 2; i++) await playWinningMove('marks-sheriff', 'sheriff');
    const final = await getUserMarks('marks-sheriff');
    expect(final.paths.sheriff.marks).toBe(4);
